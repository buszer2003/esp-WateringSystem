...
const byte PUMP_pin				= D7;		// Default D3
const byte LIGHT_pin			= D3;		// Default D7
DS1302 rtc(D4, D5, D6);
...
void callback(char* topic, byte* message, unsigned int length) {
	...
	if (strcmp(topic, "watering/pump/man") == 0) {
		bool flagManualPump = doc["flag"];
		if (flagManualPump == 1) {
			pumpManualActive = 1;
			DynamicJsonDocument docSend(128);
			String MQTT_STR;
			String MSG = "\nManual pump on (" + String(dsHr) + ":" + String(dsMin) + ")";
			docSend["status"] = "OK";
			docSend["message"] = "Manual pump on (" + String(dsHr) + ":" + String(dsMin) + ")";
			serializeJson(docSend, MQTT_STR);
			client.publish("watering/savestatus", MQTT_STR.c_str());
			sendLine(MSG);
		}
	...
	if (strcmp(topic, "watering/mois") == 0) {
		...
		if (flagLowLimitMois == 1) {
			...
			docSend["status"] = "OK";
			docSend["message"] = "Moisture low limit (" + String(lowLimitMois*10) + ")";
			serializeJson(docSend, MQTT_STR);
			client.publish("watering/savestatus", MQTT_STR.c_str());
			sendLine(MSG);
		}
...
void reconnect() {
    if (!client.connected()) {
        ...
        if (client.connect("ESP8266Client")) {
            ...
            client.subscribe("watering/pump/man");
